...
import cn.cqut.lgqs.wx.service.CaptchaCodeManager;
...
//        notifyService.notifySmsTemplate(phoneNumber, NotifyType.CAPTCHA, new String[]{code});
    ...
    public Object resetPhone(@LoginUser Integer userId, @RequestBody String body, HttpServletRequest request) {
        ...
        List<LgqsUser> userList = userService.queryByMobile(mobile);
        LgqsUser user = null;
        if (userList.size() > 1) {
    ...
    public Object bindMail(@LoginUser Integer userId, @RequestBody String body) {
    	if (userId == null) {
            return ResponseUtil.unlogin();
        }
    	LgqsUser user = userService.findById(userId);
